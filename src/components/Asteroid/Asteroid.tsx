...
export const Asteroid:FunctionComponent<{isInOrder?: boolean; className?: string, object: INearEarthObject, isLunar?: boolean, isOrdered?: boolean, addOrder: (object: INearEarthObject) => void }> =
  ({object, className, isLunar = true, isOrdered, addOrder, isInOrder}) => {
  ...
  {fullVisible && <><button className={Asteroid__.FullBack} onClick={closeFull}></button>
    <div className={Asteroid__.Full}>
        ...
        <ol>
            {
                close_approach_data.map(({relative_velocity: { kilometers_per_second}, miss_distance: {kilometers}, close_approach_date_full}, i) => <li key={i}>
                    <p>{`Скорость относительно земли: ${Math.round(Number(kilometers_per_second))} км/с`}</p>
                    <p>{`Время максимального сближения с Землёй: ${close_approach_date_full}`}</p>
                    <p>{`расстояние до Земли по орбите вокруг чего летит.: ${Math.round(Number(kilometers))} км`}</p>
                </li>)
            }
        </ol>
  </div></>}
